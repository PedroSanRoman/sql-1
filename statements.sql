...
SELECT nombre FROM usuarios_lenguajes WHERE lenguaje = 'JavaScript';
...
SELECT COUNT(DISTINCT edad) AS total_edades_diferentes FROM usuarios_lenguajes WHERE lenguaje = 'Java';
...
SELECT * FROM usuarios_lenguajes ORDER BY nombre ASC;
...
SELECT nombre, COUNT(DISTINCT lenguaje) AS cantidad_lenguajes FROM usuarios_lenguajes WHERE edad > 25 GROUP BY nombre;


-- Ejercicio 25: Seleccionar los usuarios que tienen exactamente la misma edad.
SELECT edad, GROUP_CONCAT(nombre) AS misma_edad FROM usuarios_lenguajes GROUP BY edad HAVING COUNT(*) > 1;
...
SELECT COUNT(nombre), GROUP_CONCAT(nombre) FROM usuarios_lenguajes WHERE apellido LIKE 'G%';
...
SELECT nombre, lenguaje FROM usuarios_lenguajes WHERE edad > 30 ORDER BY LENGTH(lenguaje) ASC LIMIT 1;


-- Ejercicio 31: Seleccionar los usuarios que saben al menos un lenguaje y ordenarlos por nombre de lenguaje. 
SELECT nombre FROM usuarios_lenguajes WHERE lenguaje IS NOT NULL ORDER BY lenguaje DESC;
...
SELECT nombre, edad, AVG(edad) OVER() AS edad_promedio, edad - AVG(edad) OVER() AS diferencia_edad FROM usuarios_lenguajes;
